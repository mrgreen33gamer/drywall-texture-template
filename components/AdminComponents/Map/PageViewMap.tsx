...
function hexToRgba(hex: string, alpha: number): string {
  ...
  if (h.length !== 3 && h.length !== 6) return `rgba(161, 161, 170,${alpha})`;
...
export default function PageViewMap({ data, height = 380 }: PageViewMapProps) {
  ...
  useEffect(() => {
    ...
    const accent = (cs?.getPropertyValue('--admin-rail').trim() || '#a1a1aa');
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderLeft: '3px solid var(--admin-rail, #a1a1aa)',
          borderRadius: '2px',
